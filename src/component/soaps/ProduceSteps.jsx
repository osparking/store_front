...
const ProduceSteps = () => {
  const stepsInfo = [
    ...
    {
      id: 6,
      label: "정제수와 가성소다를 계량한다.",
      renderContent: (handleAccordionToggle) => (
        <ul>
          <li>
            정제수 - 526g (526 <span id="special-span">=</span> 1,326
            <span id="special-span"> - </span>800)
          </li>
          <li>
            가성소다 - 588g
            <br />
            * 올리브유 대 가성소다: 0.133
            ...
            <a
              href="javascript:;"
              onClick={() => showIngTable(handleAccordionToggle)}
            >
              근거 테이블
            </a>
          </li>
      ...
      renderContent: () => (
        <ul>
          ...
          <li>
            ...
            <br />
            (가성소다 증기의 <span className="WARNING">독성은 구토를 유발</span>
            할 수 있으니 주의할 것)
          </li>
          <li>
            가성소다수 온도가 45℃ 이상일 것이므로, 45℃ 로 식고, 투명해질 때까지
            대기 - 약 10분 소요
            <br />(<span className="WARNING">가성소다수는 독극물</span>
            이므로 취급에 주의할 것.)
          </li>
        </ul>
      ),
    ...
    {
      id: 8,
      label: "올리브유와 가성소다수를 다음 방법으로 혼합한다.",
      renderContent: () => (
        <ul>
          <li>올리브유 스탠 다라이 위에 미세 거름망을 받친다.</li>
          <li>
            가성소다수에 잔존 가능한 유리 알칼리를 걸러내기 위하여, 거름망을
            통하여 가성소다수를 아주 천천히 흘려 보낸다.
          </li>
        </ul>
      ),
    },
    {
      id: 9,
      label: "혼합액을 다음 방법으로 반 고체화한다.",
      renderContent: () => (
        <ul>
          <li>큰 실리콘 주걱으로 2,3 분 저어 약한 트레이스를 낸다.</li>
          <li>핸드 믹서를 기포가 생기지 않게 사용하여 혼합액을 섞는다.</li>
          <li>
            약 1 분간 믹서 사용 후, 혼합액 색이 뽀얘지면, 향오일을 추가한다.
          </li>
          <li>
            핸드 믹서를 사용하여 혼합액 고체화 정도를 높이는 동안 1,2 분
            간격으로 응고 정도를 검사한다.
          </li>
          <li>
            주걱을 사용하여 다라이 혼합액 표면에 별표를 그린다. 그 별표가 5초
            이상 유지되면 고체화 과정을 완료한다.
          </li>
        </ul>
      ),
    },
    {
      id: 10,
      label: "천연 분말을 비누 원액에 섞는다.",
      renderContent: () => (
        <ul>
          <li>
            혼합액을 조금(어성초 층 - 150g, 율무씨 층 - 130g) 작은(0.5L)스탠
            비이커에 소분한다. <br />
          </li>
    ...
    {
      id: 11,
      label: "반 고체 원액을 두 층으로 몰드에 담는다.",
      renderContent: () => (
        <ul>
          <li>한 몰드의 한 층으로 원액 490g을 투입한다.</li>
          <li>어성초 층 견고화를 위해 한 두 시간 기다린다.</li>
          <li>
            기다리는 동안 몰드 배가 볼록해지지 않게, 몰드 사이에 골판지를
            넣는다.
          </li>
          <li>비이커 내용물을 전체 혼합액에 넣고 완전히 섞는다.</li>
        </ul>
    ...
    {
      id: 12,
      label: "율무씨 층까지 채운 뒤, 몰드를 보온고에 보관한다.",
      renderContent: () => (
        <ul>
    ...
    {
      id: 13,
      label: "약 2일 후, 낱개 비누를 잘라낸다.",
      renderContent: () => (
        <ul>
          <li>
            층 분리 현상 방지를 위해, 보온고에서 몰드를 하니씩 꺼내며 작업한다.(
            <span className="WARNING">층 분리 현상</span>: 한 범이비누의
            어성초와 율무 층이 떨어지는 현상)
          </li>
          <li>
            비누 커터로 두께 3.48cm 인 비누를 두 개 잘라낸다.(목표 중량: 125g)
          </li>
          <li>
            두 번째 비누의 무게가 122~128g 인지 확인하고 두께를 조절하여 자른다.
          </li>
        </ul>
      ),
    },
    {
      id: 14,
      label: "낱개 비누를 건조대에 펼쳐놓고 5 주간 건조시킨다.",
      renderContent: () => (
        <ul>
  ...
  const showIngTable = (handleAccordionToggle) => {
